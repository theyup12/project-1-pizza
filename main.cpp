...
template <typename T> bool assertTrue(const string &, const T &, const T &);

int main(int argc, char const *argv[]) {
  PizzaZine top10(10);
  top10.readInFile("data.csv");
  assertTrue("Top 10 - [0].name", string("Avanti's Italian Restaurant"),
             top10[0].name);
  ...
  assertTrue("Top 50 - [24].address", string("1225 Route 9g"),
             top50[24].address);
  assertTrue("Top 50 - [29].longitude", -90.55173, top50[29].longitude);
  assertTrue("Top 50 - [49].name", string("Morina's Italian Restaurant"),
             top50[49].name);

  PizzaZine top200(200);
  top200.readInFile("data.csv");
  assertTrue("Top 200 - [199].city", string("Addison"), top200[199].city);
  assertTrue("Top 200 - [198].name",
             string("Nordstrom The Plaza At King of Prussia"),
             top200[198].name);
  ...
  assertTrue("Top 400 - [398].address", string("1424 Morris Ave"),
             top400[398].address);
...
bool assertTrue(const string &nameOfTest, const float &expected,
                float &actual) {
  if (expected - actual < 0.01 || expected - actual > 0.01) {
    // Green colored text
    cout << "\033[32m"
         << "PASSED "
         << "\033[0m" << nameOfTest << ": expected and actual " << actual
         << endl;
    testCount++;
    return true;
  }
  ...
  cout << "\033[31m"
       << "FAILED "
       << "\033[0m" << nameOfTest << ": expected " << expected << " but actual "
       << actual << endl;
  return false;
}
...
bool assertTrue(const string &nameOfTest, const T &expected, const T &actual) {
  ...
  cout << "\033[31m"
       << "FAILED "
       << "\033[0m" << nameOfTest << ": expected " << expected
